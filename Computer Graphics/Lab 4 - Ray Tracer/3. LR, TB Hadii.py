...
#def render(camera, light, objects, rays, res):    #rast va chap
    image = np.zeros((res[0], res[1], 3))
    left_color = np.array([204, 0, 204]) / 255.0  # Replace with the RGB values of the left side color
    right_color = np.array([0, 0, 255]) / 255.0   # Replace with the RGB values of the right side color
...
#def render(camera, light, objects, rays, res):   #bala va paeen
    image = np.zeros((res[0], res[1], 3))
